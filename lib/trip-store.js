...
function defaultCreatePool(connectionString) {
  ...
  // connectionTimeoutMillis only bounds connection acquisition; the two
  // statement/query timeouts make sure a stalled query cannot hang a request
  // (the client's share/fetch calls give up after 8s).
  const pool = new Pool({
    connectionString,
    max: 3,
  ...
  // pg.Pool emits "error" for idle clients dropped by the server (restart,
  // failover, Neon autosuspend). Without a listener that event crashes the
  // whole process; trip sharing should degrade, not take the app down.
  pool.on("error", (error) => {
    console.error(`[trips] idle database connection error: ${error.message}`);
  });
  return pool;
}
